...
- (BOOL)openReadStream
{
	@synchronized(self)
	{
        ...
        //我周晓栋加上的,下面的三行代码可以附加Cookie
        CFHTTPMessageSetHeaderFieldValue(message,CFSTR("Content-Encoding"),(CFStringRef)[NSString stringWithFormat:@"gzip"]);
        CFHTTPMessageSetHeaderFieldValue(message, (CFStringRef)@"Cookie", (CFStringRef)[NSString stringWithFormat:@"qqmusic_fromtag=12 qqmusic_key=34567890 qqmusic_uin=34567890"]);
        CFHTTPMessageSetHeaderFieldValue(message,CFSTR("Connection"),(CFStringRef)[NSString stringWithFormat:@"Keep-Alive"]);
		...
		{
			CFHTTPMessageSetHeaderFieldValue(message, CFSTR("Range"),(CFStringRef)[NSString stringWithFormat:@"bytes=%ld-%ld",seekByteOffset, fileLength]);
			discontinuous = YES;
		}
